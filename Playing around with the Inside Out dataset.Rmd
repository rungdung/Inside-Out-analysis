...
[Find the Google Sheets spreadsheet here](https://docs.google.com/spreadsheets/d/1ls_jP4TAMgCwr91nPI03ufJF5uhje4n9egQKB2qRrlk/edit?usp=sharing)
...
-   I initially started by using the Closed Captions file to determine and categorise individual dialogue. Learnt within a week that it was painstakingly slow.
...
j<-ggraph(graph=i, layout="kk")+
  ...
  geom_node_text(aes(label = name), size = 1 + 0.1*degree(i), color=rgb(.1,.1,.1,.7),
                 repel = F) 
j
...
j<-ggraph(graph=i, layout="circle")+
  ...
  geom_node_text(aes(label = name), size = 1 + 0.1*degree(i), color=rgb(.8,.8,.8,.7), repel = F) +
